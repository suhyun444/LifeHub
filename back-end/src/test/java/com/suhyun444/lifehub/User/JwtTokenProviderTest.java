...
class JwtTokenProviderTest {
    ...
    void getUserIdFromToken_ValidToken() {
        ...
        String token = jwtTokenProvider.generateToken(new User(1L,email));

        // when
        String extractedId = jwtTokenProvider.getUserIdFromToken(token);

        // then
        assertThat(extractedId).isEqualTo("1");
    }
    ...
    void getUserIdFromToken_DifferentUsers() {
        // given
        String token1 = jwtTokenProvider.generateToken(new User(1L,"user1@test.com"));
        String token2 = jwtTokenProvider.generateToken(new User(2L,"user2@test.com"));
