...
class TransactionServiceTest {
    ...
    void uploadAndParseExcel_Success() throws Exception {
        ...
        // when
        List<TransactionDto> result = transactionService.uploadAndParseExcel(file, userId);
    ...
    void uploadAndParseExcel_EmptyFile() {
        ...
        assertThrows(IllegalArgumentException.class, () -> 
            transactionService.uploadAndParseExcel(emptyFile, userId));
        
        assertThrows(IllegalArgumentException.class, () -> 
            transactionService.uploadAndParseExcel(null, userId));
    }
    ...
    void uploadAndParseExcel_UserNotFound() throws IOException {
        ...
        assertThrows(NoSuchElementException.class, () -> 
            transactionService.uploadAndParseExcel(file, userId));
    }
    ...
    void getTransactions_UserNotFound() {
        ...
        assertTrue(result.isEmpty()); // 예외 대신 빈 리스트가 반환되는지 검증
    }
    ...
    void clearTransactions_UserNotFound() {
        ...
        // When & Then
        // 1. 에러가 터지지 않고 부드럽게 넘어가는지 팩트 체크
        assertDoesNotThrow(() -> transactionService.clearTransactions(invalidUserId));
        
        // 2. (선택적 꼼꼼함) 실제로 repository의 삭제 메서드가 1번 호출되긴 했는지 Mockito로 검증
        verify(transactionRepository, times(1)).deleteByUserId(invalidUserId);
    }
    ...
    void getMonthlyAnalysis_NewHistory() {
        ...
        // when
        transactionService.getMonthlyAnalysis(userId, req);
    ...
    void getMonthlyAnalysis_NoTransactions() {
        ...
        // when & then
        assertThrows(IllegalArgumentException.class, () -> 
            transactionService.getMonthlyAnalysis(1L, req));
    }
    ...
    void getAnalysis_Success() {
        ...
        // when
        List<AnalysisDto.Response> result = transactionService.getAnalysis(userId);
    ...
    void getAnalysis_HistoryNotFound() {
        ...
        // when & then
        assertThrows(NoSuchElementException.class, () -> transactionService.getAnalysis(1L));
    }
